...
/**
 * Small UI helper to render a loan status chip with consistent styling.
 */
...
/**
 * Admin/Analyst loan queue page.
 *
 * Responsibilities:
 *  - Fetch paginated loans from backend (Spring Page)
 *  - Filter by status (default: SUBMITTED for quick review)
 *  - Allow approve/reject actions on SUBMITTED loans
 *  - Show success/error feedback and loading states
 */
export default function AdminLoansPage() {
  const [loading, setLoading] = useState(true);
  const [busyId, setBusyId] = useState<number | null>(null); // prevents multiple actions at once
  ...
  /**
   * Fetches loan list based on current filter and updates local state.
   * Uses Spring Page format: { content: [], totalElements, ... }
   */
  ...
  // Simple derived counts used for the summary chips
  ...
  /**
   * Approves a loan and refreshes the list.
   * Action is only enabled for SUBMITTED loans.
   */
  ...
  /**
   * Rejects a loan and refreshes the list.
   * Action is only enabled for SUBMITTED loans.
   */
  ...
  return (
    ...
    >
      ...
      {/* Scrollable content area (header stays fixed) */}
      <Box
        sx={{
          flex: 1,
          overflowY: "auto",
                ...
                {rows.map((r) => (
                  ...
                  >
                    ...
                    >
                      ...
                      {/* Action buttons are enabled only for SUBMITTED loans */}
